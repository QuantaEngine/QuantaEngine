...
class TheoryAgent:
    ...
    def attack(self, target: TheorySpec) -> list[ChallengeCard]:
        ...
        target_seed = engines.seed_vector(target)
    ...
    def _attack_fragility(self, target, seed) -> list[ChallengeCard]:
        profile = fragility_profile(seed, lambda v: engines.assess(target, v).score, epsilon=0.05)
        if not profile:
            return []
    ...
    def _attack_overfitting(self, target) -> list[ChallengeCard]:
        ...
        base = np.array(engines.seed_vector(target).to_normalized())
        feasible = 0
        for _ in range(6):
            probe = np.clip(base + rng.normal(0, 0.25, size=base.shape), 0, 1)
            if engines.assess(target, ParameterVector.from_normalized(list(probe))).feasible:
                feasible += 1
        gp = feasible / 6
        if gp >= 0.34:
            return []
    ...
    def _attack_underconstrained(self, target, seed) -> list[ChallengeCard]:
        out = engines.assess(target, seed)
