...
class Verifier:
    ...
    def reproduce(self, target: TheorySpec, challenge: ChallengeCard) -> VerificationResult:
        ...
        if challenge.probe_vector is not None:
            ...
            out = engines.assess(target, probe)
            seed_out = engines.assess(target, engines.seed_vector(target))
            measured = {
    ...
    def benchmark(self, theory: TheorySpec) -> dict[str, float | bool]:
        ...
        std = engines.assess(theory, seed)
        s_alpha = engines.assess(theory, big_alpha).score
        s_grav = engines.assess(theory, huge_gravity).score
        return {
...
def _generative_power_quick(theory: TheorySpec, samples: int = 5) -> float:
    ...
    for _ in range(samples):
        ...
        if engines.assess(theory, ParameterVector.from_normalized(list(probe))).feasible:
            ok += 1
    return ok / samples
