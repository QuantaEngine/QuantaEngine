...
from .theory import TheoryRegistry, TheorySpec
...
def run_tournament(
    ...
    archive_features = [
        list(t.seed_vector or scoring.engines.seed_vector(t).values) for t in current
    ]
    for s, t in zip(scores, current, strict=True):
        feats = list(t.seed_vector or scoring.engines.seed_vector(t).values)
