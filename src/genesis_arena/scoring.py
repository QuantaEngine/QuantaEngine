...
from cosmogenesis.critique import fragility_profile
...
def _benchmark_fit(theory: TheorySpec) -> float:
    ...
    out = engines.assess(theory, engines.seed_vector(theory))
    return out.score
...
def _generative_power(theory: TheorySpec, samples: int = 6) -> float:
    ...
    base = np.array(engines.seed_vector(theory).to_normalized())
    feasible = 0
    for _ in range(samples):
        probe = np.clip(base + rng.normal(0, 0.2, size=base.shape), 0, 1)
        vec = ParameterVector.from_normalized(list(probe))
        if engines.assess(theory, vec).feasible:
            feasible += 1
    return feasible / samples


def _robustness(theory: TheorySpec) -> float:
    seed = engines.seed_vector(theory)
    profile = fragility_profile(seed, lambda v: engines.assess(theory, v).score, epsilon=0.05)
...
def _simplicity(theory: TheorySpec) -> float:
    ...
    out = engines.assess(theory, engines.seed_vector(theory))
    free = float(out.diagnostics.get("free_parameters", len(engines.seed_vector(theory).values)))
...
def score_theory(
    ...
    seed = engines.seed_vector(theory)
    out = engines.assess(theory, seed)
